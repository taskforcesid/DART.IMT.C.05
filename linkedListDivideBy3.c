...
created in the lessons up to this point. As before you are provided 
...
stored in this linked list of digits is divisible by three. The function 
should return the value 1 if indeed the number is divisible by three and 
it should return the value 0 otherwise.
...
int sumOfDigits(struct digit *start);
...
int sumOfDigits(struct digit *start){
    if(start->next == NULL){
        ...
        return start->num + sumOfDigits(start->next);
    }
}
